...
namespace NeoCoco
{
	public class DFA
	{
		...
		private string Ch(int ch)
		{
			...
			else return String.Format($"'{(char)ch}'");
		}

		private string ChCond(char ch)
		{
			return String.Format($"ch == {Ch(ch)}");
		}
		...
		void NewTransition(State from, State to, NodeType typ, int sym, int tc)
		{
			var t = new Target(to);
			var a = new Action(typ, sym, tc);
			a.target = t;
			from.AddAction(a);
			if (typ == NodeType.Class) curSy.tokenKind = Symbol.classToken;
		}
			...
			for (var state = firstState; state != null; state = state.next)
			{
				for (var a = state.firstAction; a != null; a = a.next)
				{
					...
					while (b != null)
					{
						...
						{
							var seta = a.Symbols(tab);
							var setb = b.Symbols(tab);
							seta.Or(setb);
		...
		void Step(State from, Node p, BitArray stepped)
		{
			...
			switch (p.Type)
			{
				...
				case NodeType.Character:
					{
						NewTransition(from, TheState(p.next), p.Type, p.val, p.code);
						break;
					}
		...
		void NumberNodes(Node p, State state, bool renumIter)
		{
			...
			if (state == null || (p.Type == NodeType.Iteration && renumIter)) state = NewState();
			p.state = state;
		...
		public void MatchLiteral(string s, Symbol sym)
		{
			...
			{ // make new DFA for s[i..len-1], ML: i is either 0 or len
				State to = NewState();
				NewTransition(state, to, NodeType.Character, s[i], Node.normalTrans);
				state = to;
			}
		...
		{
			CharSet seta, setb;
			if (a.Type == NodeType.Character)
				if (b.Type == NodeType.Character) return a.sym == b.sym;
				else { setb = tab.CharClassSet(b.sym); return setb[a.sym]; }
			else
			{
				seta = tab.CharClassSet(a.sym);
				if (b.Type == NodeType.Character) return seta[b.sym];
		...
		public void PrintStates()
		{
			...
			for (State state = firstState; state != null; state = state.next)
			{
				...
				for (Action action = state.firstAction; action != null; action = action.next)
				{
					...
					if (action.Type == NodeType.Class) trace.Write(tab.classes[action.sym].Name);
		...
		public Action FindAction(State state, char ch)
		{
			for (Action a = state.firstAction; a != null; a = a.next)
			{
				if (a.Type == NodeType.Character && ch == a.sym)
				{
					return a;
				}
				else if (a.Type == NodeType.Class)
				{
		...
		{
			gen.WriteLine();
			gen.Write($"\tbool Comment{i}() "); gen.WriteLine("{");
		...
		void GenLiterals()
		{
			gen.WriteLine($"\t\tvar value = lastToken.val{(ignoreCase ? ".ToLower()" : "")};");
			gen.WriteLine("\t\tswitch (value) {");
			foreach (var ts in new[] { tab.terminals, tab.pragmas })
			{
				foreach (Symbol sym in ts.Where(x => x.tokenKind == Symbol.litToken))
				{
					var name = SymName(sym);
					if (ignoreCase) name = name.ToLower();
					// sym.Name stores literals with quotes, e.g. "\"Literal\""
					gen.WriteLine($"\t\t\tcase {name}: lastToken.kind = {sym.n}; break;");
				}
			}
		...
		void WriteState(State state)
		{
			...
			for (Action action = state.firstAction; action != null; action = action.next)
			{
				...
				if (action.Type == NodeType.Character) gen.Write(ChCond((char)action.sym));
			...
			{
				gen.Write($"lastToken.kind = {endOf.n}; ");
				if (endOf.tokenKind == Symbol.classLitToken)
				{
					gen.WriteLine("lastToken.val = new String(tval, 0, tlen); CheckLiteral(); return lastToken;}");
				}
				else
				{
					gen.WriteLine("break;}");
		...
		void WriteStartTab()
		{
			for (Action action = firstState.firstAction; action != null; action = action.next)
			{
				...
				if (action.Type == NodeType.Character)
				{
		...
		public void WriteScanner()
		{
			...
			gen.WriteLine("\tconst int MaxTerminal = {0};", tab.terminals.Count - 1);
